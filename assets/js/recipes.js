...
addEventOnElements($filterTogglers, "click", function () {
  ...
  //
  const bodyOverflow = document.body.overflow;
      ...
      <div class="card-body">
        ...
        <div class="meta-wrapper">
          <div class="meta-item">
            ...
            >schedule</span
            >
            <span class="label-medium">${getTime(coockingTime).time || "<1"} ${
      getTime(coockingTime).timeUnit
    } </span>
          </div>
          <button
...
fetchData(queries || defaultQueries, (data) => {
  ...
  if (data.hits.length) {
    ...
    $loadMore.innerHTML = `
    <p class="info-text body-medium">No recipe found</p>
    `;
  }
});
